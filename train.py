...
def train_epoch(model, current_epoch, criterion, optimizer, dataset, args, log):
    ...
    for i, data in enumerate(trainloader):
        ...
        if (i % args['log_freq']) == 0:
            ...
            log.add_scalar('training_classication_loss', running_loss, epoch * num_iters + i)
...
def val_epoch(model, current_epoch ,criterion, val_dataset, args):
    total_loss = 0
    val_loader = torch.utils.data.DataLoader(r2r_train_dataset, batch_size=args['batch_size'], shuffle=True,
                                             num_workers=0)
...
if __name__ == '__main__':
    ...
    r2r_val_dataset = RealMaskedFacesDataset_Classification(split_file='train.txt', transforms=val_transform)
    ...
    for epoch in range(args['num_epoch']):
        ...
        val_loss, val_acc = val_epoch(model, epoch, r2r_val_dataset, args)
        ...
        log.add_scalar('val_acc_epoch', val_acc, epoch)
        log.add_scalar('val_loss_epoch', val_loss, epoch)
        log.close()
